...
describe("palette share hashes", () => {
  ...
  it("accepts 1 to 10 colors, including simple images with fewer distinct colors", () => {
    ...
    expect(decodePaletteHash(hashOf(1))).toHaveLength(1);
    expect(decodePaletteHash(hashOf(2))).toHaveLength(2);
    expect(decodePaletteHash(hashOf(3))).toHaveLength(3);
    expect(decodePaletteHash(hashOf(0))).toBeNull();
